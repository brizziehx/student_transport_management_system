<?php 
    ...
    if(isset($_POST['submit'])) {
        ...
        $notification = $_SESSION['lname']." location has been created successfully";
        if(!empty($lname) && !empty($time) && !empty($time2)) {
            $sql = $conn->query("INSERT INTO stop (locationName, time, time2, routeID) VALUES ('$lname', '$time', '$time2', '$routeIDs')");
            ...
            if($sql == true) {
                ...
                $_SESSION['msg'] = "<script>vt.success('Location has been created successfully', {position: 'top-center'})</script>";
            }
        } else {
            $_SESSION['msg'] = "<script>vt.error('All input fields are required', { title: 'Error', position: 'top-center'})</script>";
        }

    }

    $locations = $conn->query("SELECT * FROM stop WHERE routeID = '$routeIDs'");
    ...
    <main class="home grid">
        ...
        <div class="cards grid">
            ...
            <div href="#" class="card xpand2">
                ...
                <p class="flex"><b>All Locations</b><i class="bx bx-current-location icon"></i></p>
        ...
        <form class="register" method="POST" action="" enctype="multipart/form-data">
                    ...
                    <div class="input">
                        ...
                        <input type="text" name="locationName" value="<?=$_SESSION['lname'] ?? ''?>" placeholder="Location name">
                    </div>
                    <div class="input">
                        <label for="time">Pick up Time</label>
                        <input type="time" name="time" value="<?=$_SESSION['time'] ?? ''?>">
                    </div>
                    <div class="input">
                        <label for="time">Drop off Time</label>
                        <input type="time" name="time2" value="<?=$_SESSION['time2'] ?? ''?>">
                    </div>
